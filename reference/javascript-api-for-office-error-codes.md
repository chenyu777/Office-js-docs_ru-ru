...
# Коды ошибок API JavaScript для Office
В этой статье описаны сообщения об ошибках, которые могут появляться при использовании API JavaScript для Office (файла Office.js).

 _**Область применения:** Access apps for SharePoint | apps for Office | Excel | Office Add-ins | Outlook | PowerPoint | Project | Word_


## Коды ошибок

В представленной ниже таблице перечислены коды и названия ошибок, отображаемые сообщения и условия, в которых они появляются.



|**[Error.code](../reference/shared/error.code.md)**|**[Error.name](../reference/shared/error.name.md)**|**[Error.message](../reference/shared/error.message.md)**|**Условие**|
|:-----|:-----|:-----|:-----|
...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается применить метод  **addRowsAsynch** или **deleteAllDataValuesAsynch** объекта **TableBinding** к данным с типом приведения _матрица_.|
...
|5000|Ошибка устаревших параметров|Операция не поддерживается.|Операция не поддерживается используемым ведущим приложением. Например, метод  **document.getSelectionAsync** вызывается из Outlook.|
...
|5004|Недопустимый вызов API|Недопустимый вызов API в текущем контексте.|Выполнен вызов, недопустимый для контекста, например попытка использовать объект  **CustomXMLPart** в Excel.|
...
## Условия ошибок создания привязки

При создании привязки в API разработчик решения должен указать нужный тип привязки. В следующих таблицах приводится сводка возможных вариантов и ожидаемого поведения привязок.


### Поведение в Excel

В следующей таблице описано поведение привязок в Excel.



|**Указанный тип привязки**|**Выделенный фрагмент**|**Поведение**|
|:-----|:-----|:-----|
|Матрица|Диапазон ячеек (в том числе части таблиц и отдельные ячейки)|Для выбранных ячеек создается привязка типа  _матрица_.Не ожидается никаких изменений документа.|
|Матрица|Текст в ячейке|Для всей ячейки создается привязка типа  _матрица_.Не ожидается никаких изменений документа.|
|Матрица|Несколько фрагментов или недопустимое выделение (например, пользователь выделяет изображение, объект, Word Art и т. д.)|Создать привязку невозможно.|
...
В следующей таблице описано поведение привязок в Word.
...
|Таблица|Вся таблица|Создается привязка типа  _текст_.|
