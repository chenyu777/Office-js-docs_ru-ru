...
# Метод CustomXmlPrefixMappings.addNamespaceAsync
Асинхронно добавляет префикс к сопоставлению пространства имен для использования при запрашивании элемента.
...
|:-----|:-----|:-----|:-----|
| _prefix_|**string**|Задает префикс, добавляемый в список сопоставления префиксов. Обязательный.||
| _ns_|**string**|Задает пространство имен, назначаемое добавляемому префиксу. Обязательный.||
...
Если функция обратного вызова передана методу **addNamespaceAsync**, вы можете использовать свойства объекта **AsyncResult**, чтобы получить следующие сведения:
