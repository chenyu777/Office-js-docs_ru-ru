...
# Метод ProjectDocument.getTaskAsync
Асинхронно получает имя указанной задачи, назначенные ресурсы и идентификатор задачи в синхронизированном списке задач SharePoint.
...
В случае метода **getTaskAsync** возвращенный объект [AsyncResult](../../reference/shared/asyncresult.md) содержит следующие свойства:
...
|[value](../../reference/shared/asyncresult.value.md)|Содержит следующие свойства:<br/><br/><ul><li><b>taskName</b> — название задачи.</li><li><b>wssTaskId</b> — идентификатор задачи в синхронизированном списке задач SharePoint. Если проект не синхронизирован со списком задач, значение будет <b>0</b>.</li><li><b>resourceNames</b> — список имен ресурсов, назначенных задаче (через запятую).</li></ul>|

## Замечания

Перед вызовом метода **getTaskAsync** вызовите метод [getSelectedTaskAsync](../../reference/shared/projectdocument.getselectedtaskasync.md), чтобы получить GUID задачи.


## Пример

Ниже приведен пример кода, который вызывает метод [getSelectedTaskAsync](../../reference/shared/projectdocument.getselectedtaskasync.md), чтобы получить GUID выбранной задачи. Затем он вызывает метод **getTaskAsync**, чтобы получить свойства задачи, доступные в API JavaScript для Office.
...
[Метод getSelectedTaskAsync](../../reference/shared/projectdocument.getselectedtaskasync.md)
