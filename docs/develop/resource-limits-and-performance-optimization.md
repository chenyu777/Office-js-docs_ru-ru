...
-  **Загрузка ядра ЦП**. Загрузка одного ядра ЦП не должна превышать 90 % (по умолчанию проверяется три раза в течение каждых 5 секунд).
...
-  **Устойчивость к сбоям**. Предельное значение по умолчанию — четыре сбоя надстройки.
...
-  **Блокирование надстройки**. Отсутствие ответа должно длиться не более 5 секунд.
    ...
    Помимо правил, связанных с памятью, надежностью и ядром ЦП, надстройки Outlook должны соблюдать при активации следующие правила:
      ...
      -  **Время ответа при оценке регулярных выражений**. По умолчанию Outlook должна оценивать все регулярные выражения в манифесте надстройки Outlook не более 1000 миллисекунд. При превышении этого значения Outlook повторит оценку позже.
        ...
        С помощью групповой политики или предназначенного для конкретного приложения параметра в реестре Windows администраторы могут настраивать это пороговое значение (по умолчанию оно составляет 1000 миллисекунд) в параметре **OutlookActivationAlertThreshold**. Дополнительные сведения см. в статье [Переопределение параметров использования ресурсов для повышения производительности надстроек Office](http://msdn.microsoft.com/library/da14ec8c-5075-4035-a951-fc3c2b15c04b%28Office.15%29.aspx).
...
## Проверка проблем использования ресурсов в журнале телеметрии
...
|**Дата, время**|**Код события**|**Severity**|**Должность**|**Файл**|**ID**|**Приложение**|
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
...
|**Код события**|**Должность**|**Severity**|**Описание**|
|:-----|:-----|:-----|:-----|
...
Дополнительные сведения см. в разделах [Развертывание информационной панели телеметрии](http://msdn.microsoft.com/en-us/library/f69cde72-689d-421f-99b8-c51676c77717%28Office.15%29.aspx) и [Устранение неполадок с файлами Office и пользовательскими решениями с помощью журнала телеметрии Office](http://msdn.microsoft.com/library/ef88e30e-7537-488e-bc72-8da29810f7aa%28Office.15%29.aspx)
...
- В сценарии, где надстройке требуется читать большой объем из неограниченного набора данных, можно применить разбиение на страницы во время чтения данных из таблицы или уменьшить размер данных в каждой небольшой операции чтения вместо попыток выполнить чтение за одну операцию. 
    ...
    Пример кода JavaScript и jQuery, иллюстрирующий разбитие потенциально длительной и ресурсоемкой серии операций ввода-вывода с неограниченными данными, см. в статье [Как временно вернуть управление браузеру во время интенсивной обработки JavaScript](http://stackoverflow.com/questions/210821/how-can-i-give-control-back-briefly-to-the-browser-during-intensive-javascript). В этом примере используется метод [setTimeout](http://msdn.microsoft.com/en-us/library/ie/ms536753%28v=vs.85%29.aspx) глобального объекта, чтобы ограничить продолжительность операций ввода и вывода. Кроме того, обрабатываются данные с разбивкой на конкретные фрагменты, а не данные неограниченного размера в произвольном порядке.
    
- Если надстройка использует требовательный к ресурсам ЦП алгоритм для обработки большого объема данных, вы можете использовать рабочие веб-процессы для выполнения длительной задачи в фоновом режиме, при этом на переднем плане выполняя отдельный скрипт, который, например, отображает ход выполнения в пользовательском интерфейсе. Рабочие веб-процессы не блокируют действия пользователя и позволяют HTML-странице отвечать на запросы. Пример рабочих веб-процессов можно посмотреть в статье [Основные сведения о рабочих веб-процессах](http://www.mdl5rocks.com/en/tutorials/workers/basics/). Дополнительные сведения об API рабочих веб-процессов в Internet Explorer изложены в статье [Рабочие веб-процессы](http://msdn.microsoft.com/en-us/library/IE/hh772807%28v=vs.85%29.aspx).
