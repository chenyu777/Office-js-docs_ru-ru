...
# Проверка маркера удостоверения с использованием PHP

Надстройка Outlook может отправить пользователю маркер удостоверения, но прежде чем ответить на запрос, необходимо убедиться, что маркер поступил с нужного сервера Exchange Server. В примере, приведенном в статье, для проверки маркера удостоверения используется PHP, но вы можете использовать для этого любой язык программирования. Действия, необходимые для проверки маркера, описаны в статье [Проект интернет-документа в формате JSON Web Token (JWT)](http://self-issued.info/docs/draft-goland-json-web-token-00.html).

Рекомендуется использовать процесс, состоящий из четырех этапов, для проверки маркера удостоверения и получения уникального идентификатора пользователя: 


1. Извлечь JWT из URL-кодированной строки base64.
    
2. Проверить целостность маркера, то есть его предназначение для этой надстройки Outlook, его актуальность и возможность извлечения допустимого URL-адреса для проверки подлинности метаданных документа.
    
3. Получить документ проверки подлинности метаданных с сервера Exchange и проверить подпись, прилагаемую к маркеру удостоверения.
    
4. Вычислить уникальный идентификатор пользователя с помощью хэширования идентификатора Exchange пользователя с URL-адресом документа проверки подлинности метаданных.
    
В целом процесс может казаться очень сложным, однако каждый отдельный этап довольно прост. Можно убедиться, что библиотека PHP разбивает процесс на описанные действия в проверочных целях.
...
## Использование библиотеки проверки маркеров

Следующий код является библиотекой PHP для проверки маркера удостоверения Exchange.
